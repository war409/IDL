...
PRO convert_mm_to_percent
  time = SYSTIME(1) 
  PRINT,''
  PRINT,'Start Processing: convert_mm_to_percent'
  PRINT,''
  
  prefix = 'rain.changegrid.'
  directory_out = 'G:\projects\saf\awap\rain\2000\change_revised\' 
  
  
  directory_one = 'G:\projects\saf\awap\rain\2000\5km\'
  ...
  FOR i = 0, N_ELEMENTS(filenames_one)-1 DO BEGIN
    ...
    ; Convert rainfall from mm per day to mm per month.
    data_one = data_one * 31
;    data_one = data_one * 30
;    data_one = data_one * 28.25
    
    ; Calculate the difference between the climate estimates.
    difference = data_one - data_two
    
    ; Calculate the difference between the climate estimates as a percentage of change between data_two and data_one.
    data_out = (100 * difference) / data_two
