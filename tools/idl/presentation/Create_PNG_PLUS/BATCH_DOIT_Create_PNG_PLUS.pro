...
PRO BATCH_DOIT_Create_PNG_PLUS
  ...
  PATH = 'H:\Projects\NWC_Groundwater_Dependent_Ecosystems\Gamma'
  ...
  ; SET BACKGROUND IMAGE SET
  
  
  ; SET TRANSPARANCY VALUE (OR VALUE RANGE)
  ...
  FOR i=0, N_ELEMENTS(IN_FILES)-1 DO BEGIN ; FOR i
    ...
    IF FINITE(NAN_VALUE) EQ 1 THEN BEGIN
      ...
      IMAGE_CURRENT = TVREAD(XSTART, YSTART, NCOLS, NROWS) ; RETURN DISPLAY WINDOW AS 24BIT IMAGE
      ;-----------------------------------
    ENDIF
    ;-------------------------------------------------------------------------------------------  
    ...
    ; ADD BACKGROUND IMAGE - TRANSPARENT FOREGROUND
    B_VALUE = 0 ; TEMP
    B_TYPE = 0 ; TEMP
    IF (B_VALUE EQ 0) AND (B_TYPE EQ 0) THEN BEGIN
      IF FINITE(NAN_VALUE) EQ 1 THEN IMAGE_CURRENT = IMAGE_CURRENT ELSE IMAGE_CURRENT = IMAGE_A
      ; DRAW BACKGROUND:
      ...
      DATA_RM = ENVI_GET_DATA(FID=FID_RM, DIMS=DIMS, POS=0) ; GET DATA
      DATA_GM = ENVI_GET_DATA(FID=FID_GM, DIMS=DIMS, POS=0) ; GET DATA      
      DATA_BM = ENVI_GET_DATA(FID=FID_BM, DIMS=DIMS, POS=0) ; GET DATA      
      ;--------------
      ; GET MAP INFORMATION: GET NEW POS - LATMIN ETC - FROM DIMENSIONS
      ;--------------    
      ENVI_FILE_QUERY, FID_RM, DIMS=DIMS_RM, NS=NS_RM, NL=NL_RM, NB=NB_RM, FNAME=FNAME_RM, DATA_TYPE=DT_RM
      ...
      DATA_RM = FIX((SLOPE*DATA_RM)-INTERCEPT) ; APPLY STRETCH
      DATA_RM_PNG = BYTE(TEMPORARY(DATA_RM)) ; CONVERT DATATYPE TO BYTE
      ...
      DATA_GM = FIX((SLOPE*DATA_GM)-INTERCEPT) ; APPLY STRETCH
      DATA_GM_PNG = BYTE(TEMPORARY(DATA_GM)) ; CONVERT DATATYPE TO BYTE
      ...
      DATA_BM = FIX((SLOPE*DATA_BM)-INTERCEPT) ; APPLY STRETCH
      DATA_BM_PNG = BYTE(TEMPORARY(DATA_BM)) ; CONVERT DATATYPE TO BYTE      
      ;--------------
      ; WARP THE INPUT DATA TO THE MAP SPACE
      MODIS_IMAGE = MAP_IMAGE(DATA_RM_PNG, STARTX, STARTY, XSIZE, YSIZE, COMPRESS=1, MAP_STRUCTURE=PROJ, $
        LATMIN=BOTTOM, LONMIN=LEFT, LATMAX=TOP, LONMAX=RIGHT)
    
    
    
    ;-------------------------------------------------------------------------------------------   
    ; ADD BACKGROUND IMAGE - CUT FOREGROUND
    ;IF (B_VALUE EQ 0) AND (B_TYPE EQ 1) THEN BEGIN
    ;  DATA_CUT = DATA ; SET DATA VARIABLE
    ;--------------
    ;  IF CUT_TYPE EQ 0 THEN BEGIN ; CUT EQ VALUE
    ;  IF CUT_TYPE EQ 1 THEN BEGIN ; CUT NE VALUE
    ;  IF CUT_TYPE EQ 2 THEN BEGIN ; CUT GE VALUE
    ;  IF CUT_TYPE EQ 3 THEN BEGIN ; CUT GT VALUE
    ;  IF CUT_TYPE EQ 4 THEN BEGIN ; CUT LE VALUE
    ;ENDIF 
    ENDIF
