...
PRO BATCH_DOIT_Convert_between_Multiband_and_Singleband_format
  ...
  IF CT EQ 0 THEN BEGIN ; MULTIBAND TO SINGLEBAND
    ...
    PATH='\\File-wron\Working\work\NWC_Groundwater_Dependent_Ecosystems\Data\Imagery\Landsat\Namoi'
    ...
    FOR i=0, N_ELEMENTS(IN_FILES)-1 DO BEGIN ; FOR i
      ...
      FOR j=0, N_ELEMENTS(BNAME)-1 DO BEGIN ; FOR j
        ...
        ;---------------------------------------------------------------------------------------
        ; WRITE DATA
        ENVI_DOIT, 'CF_DOIT', FID=FID, DIMS=INDIMS, POS=[j], OUT_DT=DT, OUT_NAME=FNAME_OUT, $
          R_FID=RFID, OUT_BNAME=BANDNAME, /NO_REALIZE
        ;---------------------------------------------------------------------------------------
      ENDFOR ; FOR j
      ;-----------------------------------------------------------------------------------------
  ...
  ENDIF ELSE BEGIN ; SINGLEBAND TO MULTIBAND
    ...
    ;-------------------------------------------------------------------------------------------
    ; WRITE DATA
    ENVI_DOIT, 'CF_DOIT', FID=ARRAY_FID, DIMS=INDIMS, POS=ARRAY_POS, OUT_DT=DT, $
      OUT_NAME=FNAME_OUT, OUT_BNAME=ARRAY_BNAME, R_FID=RFID, /NO_REALIZE
    ;-------------------------------------------------------------------------------------------
  ENDELSE
  ;---------------------------------------------------------------------------------------------
