...
pro Open_Water_DOIT_Logistic_Modelling_Final5
  compile_opt idl2
  ...
  for i=0, NB-1 do Landsat_Class[*,*,i] = ENVI_GET_DATA(FID=FID, DIMS=DIMS, POS=i) 
  ...
  for i=0, 3 do begin
     ...
     endcase
          expr = 'FIVE_BANDS_MODEL(X, P)'
          
          x= [[[Landsat_Reflectance[*,*,4]]],[[Landsat_Reflectance[*,*,5]]],[[Landsat_Reflectance_NDVI]],[[Landsat_Reflectance_NDWI]], $
          [[SRTM_DEM3MrVBF]]]
          ...
          TVImage, Rotate(Bytscl(FIVE_BANDS_MODEL(X, Optimised_params), MIN=-1, MAX=1),7), /Keep_Aspect, /AXES
              xyouts,0,0, 'MODELLED' 
          TVImage, Rotate(Bytscl(FIVE_BANDS_MODEL(X, Optimised_params)-y , MIN=-0.3, MAX=0.3),7), /Keep_Aspect, /AXES
          ...
          envi_enter_data, FIVE_BANDS_MODEL(X, Optimised_params)
          envi_enter_data, FIVE_BANDS_MODEL(X, Optimised_params) - y
          print,''
    endfor
      envi_enter_data, y
      !P = P_ 
