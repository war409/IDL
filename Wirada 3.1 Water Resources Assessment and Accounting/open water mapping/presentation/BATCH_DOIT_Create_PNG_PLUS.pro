...
PRO BATCH_DOIT_Create_PNG_PLUS
  ...
  NT = FUNCTION_WIDGET_Radio_Button('Define an Input No-Data Value?  ', ['YES', 'NO'])
  ;--------------
  IF NT EQ 0 THEN BEGIN
    ; SET THE NODATA VALUE
    NAN_VALUE = FUNCTION_WIDGET_Enter_Value('Define the Input No-Data Value:  ', 255)   
    ;--------------
    ; SET THE NODATA COLOUR
    COLOUR_NODATA = FUNCTION_WIDGET_Set_Colour('Select the No-Data Colour    ')
  ...
  ;---------------------------------------------------------------------------------------------
  ; SHAPEFILE
  VT = FUNCTION_WIDGET_Radio_Button('Add a Shapefile to the Map?  ', ['YES', 'NO'])
  ;--------------
  IF VT EQ 0 THEN BEGIN
    ; SELECT THE INPUT SHAPEFILE:
    PATH = 'C:\'  
    IN_SHAPE = DIALOG_PICKFILE(PATH=PATH, TITLE='Select the Input Shapefile', FILTER=['*.shp'], /MUST_EXIST)
    IF IN_SHAPE[0] EQ '' THEN RETURN ; ERROR CHECK:
    SHAPE = OBJ_NEW('IDLffShape', IN_SHAPE) ; OPEN THE SHAPEFILE AND CREATE THE SHAPE OBJECT
    ;--------------
    ; SET THE SHAPEFILE PARAMETERS:
    COLOUR_LINE = FUNCTION_WIDGET_Set_Colour('Select the Shapefile Line Colour')
    THICKNESS = FUNCTION_WIDGET_Enter_Value('Set the Shapefile Line Thickness:  ', 1)  
    LINESTYLE = FUNCTION_WIDGET_Radio_Button('Set the Shapefile Line Style:  ', ['0 Solid','1 Dotted','2 Dashed', $
      '3 Dash Dot','4 Dash Dot Dot','5 Long Dashes'])     
    FT = FUNCTION_WIDGET_Radio_Button('Select a Colour to Fill the Shapefile?', ['YES', 'NO'])
    IF FT EQ 0 THEN COLOUR_FILL = FUNCTION_WIDGET_Set_Colour('Select the Shapefile Fill Colour')
  ENDIF
  ;---------------------------------------------------------------------------------------------  
  ; ANNOTATION
  AT = FUNCTION_WIDGET_Radio_Button('Add Annotation to the Map?  ', ['YES', 'NO'])
  ;--------------
  IF AT EQ 0 THEN BEGIN
    AT_FNAME = FUNCTION_WIDGET_Radio_Button('Use the Input Filenames to Annotate the Map?', ['YES', 'NO'])
    IF AT_FNAME EQ 1 THEN BEGIN
      TEXT_IN = FUNCTION_WIDGET_Enter_String('  Enter Text:  ', '')
    ENDIF
    ;--------------
    ; SET THE ANNOTATION PARAMETERS:
    COLOUR_TEXT = FUNCTION_WIDGET_Set_Colour('Select the Annotation Colour  ')
    CSIZE = FUNCTION_WIDGET_Enter_Value('  Set the Character Size:  ', 1)
    CTHICK = FUNCTION_WIDGET_Enter_Value('  Set the Character Line Thickness:  ', 1)
    ORIENTATION = FUNCTION_WIDGET_Enter_Value('Set the Annotation Orientation (degrees):  ', 0.0)
    ;FONT = FUNCTION_WIDGET_Enter_String('  Select the Text Font:  ', 'Calibri')
    POSITION =  FUNCTION_WIDGET_Radio_Button('Set the Annotation Position:  ', ['0 Upper Left','1 Centre Top', $
      '2 Upper Right','3 Lower Left','4 Centre Bottom','5 Lower Right'])
  ENDIF
  ;---------------------------------------------------------------------------------------------
  ; MAP BORDER
  BT = FUNCTION_WIDGET_Radio_Button('Draw a Border Around the Map?  ', ['YES', 'NO'])
  IF BT EQ 0 THEN COLOUR_BORDER = FUNCTION_WIDGET_Set_Colour('Select the Map Border Colour')
  ;---------------------------------------------------------------------------------------------
  ; SET THE CONTRAST STRETCH STATUS
  ST = FUNCTION_WIDGET_Radio_Button('Set Fixed Display Data Limits?  ', ['YES', 'NO'])
  ;--------------
  ...
  IF ST EQ 0 THEN BEGIN
    ...
    ;--------------
    S_VALUES = FUNCTION_WIDGET_Enter_Value_Pair('Define the Display Data Limits  ', 'Minimum ', 'Maximum', 0.0000, 100.0000)
    ;--------------
    ; SET PARAMETERS
    LOW = S_VALUES[0]
    HIGH = S_VALUES[1]
  ...
  FOR i=0, N_ELEMENTS(IN_FILES)-1 DO BEGIN ; FOR i
    ...
    IF AT EQ 0 THEN BEGIN
      ...
      ;--------------
      IF AT_FNAME EQ 0 THEN ANNOTATION = FNS_IN ELSE ANNOTATION = TEXT_IN
      ;--------------
      ...
      IF POSITION EQ 1 THEN BEGIN ; Centre Top
        XPOS = MID_X-(STRLEN(ANNOTATION)*3.5)
        YPOS = END_Y-(20*CSIZE)
      END      
      IF POSITION EQ 2 THEN BEGIN ; Upper Right
        XPOS = END_X-(8*CSIZE)*STRLEN(ANNOTATION)
        YPOS = END_Y-(20*CSIZE)
      END
        ...
        IF ENTITY.shape_type EQ 5 OR ENTITY.shape_type EQ 15 OR  ENTITY.shape_type EQ 25 THEN BEGIN
          ...
          IF PTR_VALID(ENTITY.parts) THEN BEGIN
            ...
            FOR j=0, ENTITY.n_parts-1 DO BEGIN ; LOOP THROUGH EACH POLYGON
              ...
              IF FT EQ 0 THEN POLYFILL, CX, CY, COLOR=FSC_Color(COLOUR_FILL)
              PLOTS, CX, CY, COLOR=FSC_Color(COLOUR_LINE), LINESTYLE=LINESTYLE, THICK=THICKNESS              
            ENDFOR
          ENDIF
        ENDIF
        ;--------------   
          ...
          IF Ptr_Valid(ENTITY.parts) THEN BEGIN
            ...
            FOR j=0, ENTITY.n_parts-1 DO BEGIN ; LOOP THROUGH EACH POLYLINE
              PlotS, (*ENTITY.vertices)[0, CUTS[j]:CUTS[j+1]-1], (*ENTITY.vertices)[1, CUTS[j]:CUTS[j+1]-1], $
                COLOR=FSC_Color(COLOUR_LINE), LINESTYLE=LINESTYLE, THICK=THICKNESS
            ENDFOR
          ENDIF
        ENDIF
      ENDFOR
