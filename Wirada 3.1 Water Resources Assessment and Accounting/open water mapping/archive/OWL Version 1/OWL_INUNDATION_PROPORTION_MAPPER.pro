; ######################################################################
; NAME: OWL_INUNDATION_PROPORTION_MAPPER.pro
; LANGUAGE: ENVI IDL
; AUTHOR: JUAN PABLO GUERSCHMAN, MODIFIED BY GARTH WARREN
; DATE: 05/11/2009
; DLM: 11/12/2009
; DESCRIPTION: THIS TOOL CREATES AN INUNDATION ACCUMULATION (PROPORTION OF)  
;              IMAGE BASED ON USER-SELECTED PARAMETERS. INUNDATION PROPORTION
;              IS MAPPED BY-TIME-SERIES. A CELL IS CLASSIFIED AS BEING INUNDATED 
;              IF THE OWL VALUE IS EQ, GT, GE, LT OR LE A USER-SELECTED OWL 
;              THRESHOLD. THE OUTPUT - BY-CELL - IS THE PROPORTION OF TIME
;              INUNDATED, THAT IS, THE PERCENTAGE OF, INUNDATION COUNT DIVIDED 
;              BY TOTAL COUNT (NOT INCLUDING CLOUD DATES). 
...
; ######################################################################
; 
PRO OWL_INUNDATION_PROPORTION_MAPPER
  ; GET START TIME
  F_TIME = SYSTIME(1)
  PRINT,''
  PRINT,'BEGIN PROCESSING: OWL_INUNDATION_PROPORTION_MAPPER'
  ;---------------------------------------------------------------------
  ...
  ENDFOR ; END 'FOR i'
  ;---------------------------------------------------------------------
  ; CONVERT ACUM_OWL_NE255 TO FLOAT
  ACUM_OWL_NE255 *= 1.0
  ...
  ENVI_WRITE_ENVI_FILE, OWL_EXP, OUT_NAME=FNAME1, MAP_INFO=MAP_INFO, /NO_OPEN
  ;---------------------------------------------------------------------
  PRINT,''
  ...
  PRINT,''
  PRINT,'FINISHED PROCESSING: OWL_INUNDATION_PROPORTION_MAPPER'
  PRINT,'' 
END  