; ##########################################################################
; NAME: Time_Series_Analysis_GET_Residual_Difference.pro
; LANGUAGE: IDL
...
; DLM: 09/04/2010
;
; DESCRIPTION: This tool calculates the residual difference of the linear
;              regression between the model estimate (M) and the observation (O).  
;              The statistic is calculated as: RD = SQRT((1-r2)(varObs)), where
;              r2 is the squared Pearsons product moment correlation coefficient
;              and varObs is the variance of the observation calculated as: 
;              ∑(O – Obar)² / n-1
;
...
;              
;              Data is calculated on a cell-by-cell basis.
;
...
; ##########################################################################
;
PRO Time_Series_Analysis_GET_Residual_Difference
  ;-------------------------------------------------------------------------
  ; GET START TIME
  T_TIME = SYSTIME(1)
  PRINT,''
  PRINT,'BEGIN PROCESSING: Time_Series_Analysis_GET_Residual_Difference'
  PRINT,''
  ;-------------------------------------------------------------------------
  ;*************************************************************************
  ; INPUT/OUTPUT:
  ...
  IF STATUS EQ 0 THEN BEGIN
    ...
    FIELD = CW_FIELD(BASE, XSIZE=8, VALUE=-999.00, TITLE='SET THE NODATA VALUE  ', /RETURN_EVENTS)
    WIDGET_CONTROL, BASE, /REALIZE
    RESULT = WIDGET_EVENT(BASE)
  ...
  OUT_RDIFF = OUTFOLDER + 'Residual.Difference' + '.img'
  ;-------------------------------------------------------------------------
  ; CREATE THE FILES
  OPENW, UNIT_RDIFF, OUT_RDIFF, /GET_LUN
  ;-------------------------------------------------------------------------
  ; CLOSE THE NEW FILES
  FREE_LUN, UNIT_RDIFF
  ;-------------------------------------------------------------------------
  ...
  FOR s=0, COUNT_S-1 DO BEGIN ; START 'FOR s'
    ...
    STDDEV = SQRT(TRANSPOSE(TOTAL(OD, 1, /NAN)) / TRANSPOSE(TOTAL(FINITE(MATRIX_O), 1))) 
    ;-----------------------------------------------------------------------
    ; GET THE SQUARED PEARSONS PRODUCT MOMENT CORRELATION COEFFICIENT
    R2 = (CP / (SQRT(MD) * SQRT(OD)))^2
    ;-----------------------------------------------------------------------    
    ; GET THE OBSERVED VARIANCE
    ;VARO = OD / (TRANSPOSE(TOTAL(FINITE(MATRIX_O), 1)) - 1)
    VARO = ((TRANSPOSE(TOTAL(STDDEV, 1, /NAN))))^2  
    ;-----------------------------------------------------------------------    
    ; GET THE RESIDUAL DIFFERENCE:
    ;-----------------------------------------------------------------------
    ; GET 1 - PPM^2
    R2M1 = (1 - R2)
    ; GET R2M1 * VARO
    ...
    k = WHERE(R2M1tVAR0 LT FLOAT(0), COUNT_k)
    IF (COUNT_k GT 0) THEN R2M1tVAR0[k] = (R2M1tVAR0[k]) * (-1.00)
    ...
    OPENU, UNIT_RDIFF, OUT_RDIFF, /GET_LUN, /APPEND
    ;-----------------------------------------------------------------------
    ; APPEND DATA TO THE OUTPUT FILES
    WRITEU, UNIT_RDIFF, RD
    ;-----------------------------------------------------------------------
  ...
  PRINT,''
  PRINT,'FINISHED PROCESSING: Time_Series_Analysis_GET_Residual_Difference'
  PRINT,''
  ;-------------------------------------------------------------------------
END    