; ######################################################################
; NAME: RADIOSONDE_ET_TIMESERIES_EXTRACT_ARRAYMEAN.pro
; LANGUAGE: IDL
; AUTHOR: Garth Warren
; DATE: 30/10/2009
; DLM: 02/11/2009
; DESCRIPTION: THIS TOOL EXTRACTS RASTER TIME-SERIES DATA AT 31 RADIOSONDE
;              LAUNCH SITES. THE MEAN VALUE OF THE 9X9 ARRAY IS RETURNED.
; INPUT: 31 RADIOSONDE SITES (AS [9X9] ROIs). SINGLE OR MULTI-BAND IMAGE.
; OUTPUT: ONE CSV FILE.
; SET PARAMETERS: SEE **DEFINE** IN THE CODE.
; NOTES: MAKE SURE THAT THE COLUMN HEAD IS SORTED LIKE THE ENVI ROI LIST.
; ######################################################################
; 
PRO RADIOSONDE_ET_TIMESERIES_EXTRACT_ARRAYMEAN
  ; GET START TIME FOR WHOLE
  F_DATE = SYSTIME(1)
  PRINT,''
  PRINT,'BEGIN PROCESSING: RADIOSONDE_ET_TIMESERIES_EXTRACT_ARRAYMEAN'
  ;---------------------------------------------------------------------
  ...
  PRINT,''
  PRINT,'FINISHED PROCESSING: RADIOSONDE_ET_TIMESERIES_EXTRACT_ARRAYMEAN'
  PRINT,'' 
END  