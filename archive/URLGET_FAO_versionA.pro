...
;              
; INPUT:       Define the output directory, the input name file and the 
;              base URL. 
; 
...
;                          
; NOTES:       Version A uses the IDL IDLnetURL features to connect to,
;              and download data.
; 
; ##########################################################################
;
PRO URLGET_FAO_versionA
  ;-------------------------------------------------------------------------
  ; GET START TIME
  T_TIME = SYSTIME(1)
  PRINT,''
  PRINT,'BEGIN PROCESSING: URLGET_FAO_versionA'
  PRINT,''
  ;-------------------------------------------------------------------------
  ;*************************************************************************
  ; INPUT/OUTPUT:
  ...
  WHILE NOT EOF(INFILElun) DO BEGIN ; START 'WHILE 1'
    ...
    OUTFILE = GETURL->GET(FILENAME=OUTFILE, URL=FNAME2)
  ...
  PRINT,''
  PRINT,'FINISHED PROCESSING: URLGET_FAO_versionA'
  PRINT,''
  ;-------------------------------------------------------------------------
END  