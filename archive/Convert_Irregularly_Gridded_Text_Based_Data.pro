...
;
; OUTPUT:      One floating point binary (bsq) file and an associated ENVI
;              header (.hdr) file; per input data file. 
;               
...
PRO Convert_Irregularly_Gridded_Text_Based_Data
  ...
  ;-------------------------------------------------------------------------
  ; SELECT THE OUTPUT DIRECTORY
  DIR_OUT = DIALOG_PICKFILE(PATH='\\file-wron\Working\work\TerReS_team\Visitors\Beck\FASIR data\temp', $
  ...
  FOR i=0, COUNT_DF-1 DO BEGIN ; START 'FOR i'
    ...
    FOR j=0, COUNT_LINE-1 DO BEGIN
      ...
      MATRIX_M[2,SEGMENT_START:SEGMENT_END] = FLOAT(MATRIX_DATA[*,j])
      ;---------------------------------------------------------------------
    ENDFOR
    ;-----------------------------------------------------------------------    
    ...
    ; CONVERT THE DATA:
    ;-----------------------------------------------------------------------    
    ; BUILD THE OUTPUT MAP PROJECTION (ENVI)
    OUT_PROJ = ENVI_PROJ_CREATE(/GEOGRAPHIC, DATUM='WGS-84', NAME='GEOGRAPHIC Lat/Lon', UNITS=Degrees)
    ; CONVERT THE DATA
    ENVI_DOIT, 'ENVI_GRID_DOIT', /EXTRAP, OUT_NAME=FILE_OUT, R_FID=R_FID, INTERP=1, OUT_DT=4, $
      X_PTS=((MATRIX_M[1,*])[INDEX_X]), Y_PTS=((MATRIX_M[0,*])[INDEX_X]), Z_PTS=((MATRIX_M[2,*])[INDEX_X]), $
      PIXEL_SIZE=[SIZE_OUT,SIZE_OUT], O_PROJ=OUT_PROJ, /NO_REALIZE
    ;-----------------------------------------------------------------------
    ; GET END TIME: LOOP
