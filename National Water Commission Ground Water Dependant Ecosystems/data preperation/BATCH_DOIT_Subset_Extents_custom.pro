; ##############################################################################################
; NAME: BATCH_DOIT_Subset_Extents_custom.pro
; LANGUAGE: ENVI + IDL
...
PRO BATCH_DOIT_Subset_Extents_custom
  ;---------------------------------------------------------------------------------------------
  ; GET START TIME
  T_TIME = SYSTIME(1)
  ;--------------
  PRINT,''
  PRINT,'BEGIN PROCESSING: BATCH_DOIT_Subset_Extents_custom'
  PRINT,''
  ;---------------------------------------------------------------------------------------------  
  ;*********************************************************************************************
  ; INPUT/OUTPUT:
  ...
  ;---------------------------------------------------------------------------------------------  
  ; SELECT THE INPUT:
  PATH='\\File-wron\Working\work\NWC_Groundwater_Dependent_Ecosystems\Data\Raster'
  ...
  FOR i=0, N_ELEMENTS(IN_X)-1 DO BEGIN ; START 'FOR i'
    ...
    ;--------------
    ; ADD EMPTY ROWS TO TOP OF IMAGE:
    ;  B = MAKE_ARRAY(3, /BYTE, VALUE = 255)  
    ;  A = MAKE_ARRAY(3, /BYTE, VALUE = 100) 
    ;  C = MAKE_ARRAY(6, /BYTE) 
    ;  C[0:2] = B
    ;  C[3:5] = A
    
    DATA_TMP = MAKE_ARRAY(8949, 7246, /BYTE, VALUE = 255)
    
    DATA_TMP[0:8948,6:7239] = DATA_IN
    ...
    MAPINFO_OUT = ENVI_MAP_INFO_CREATE(DATUM=DATUM_X, NAME=PROJ_X, PROJ=PROJFULL_X, PS=[XSIZE_X, YSIZE_X], $
      ...
      CYUL_X + (DCOMPLEX(6) * DCOMPLEX(YSIZE_X))], UNITS=UNITS_X)
    ;--------------------------------------    
    ; WRITE DATA
    ENVI_WRITE_ENVI_FILE, DATA_TMP, OUT_NAME=FILE_OUT, MAP_INFO=MAPINFO_OUT, $
      PIXEL_SIZE=[XSIZE_X, YSIZE_X], OUT_DT=DT_OUT, NS=8949, NL=7246, $
