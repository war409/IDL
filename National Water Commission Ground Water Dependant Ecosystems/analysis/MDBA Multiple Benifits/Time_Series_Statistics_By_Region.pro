...
PRO Time_Series_Statistics_By_Region
  ...
  Statistic_Index = WHERE(In_Statistic.LIST EQ 1) 
  ...
  File_Header = MAKE_ARRAY((N_ELEMENTS(ROI_Names)*(2+N_ELEMENTS(Statistic_String)))+6, /STRING) ; Create an array to store the output file header.
  ...
  File_Header[2] = 'DD/MM/YYYY' ; Set the third element in the array (the date of the i-th input).  
  File_Header[3] = 'Total_CellCount'
  File_Header[4] = 'Total_NaNCount'
  File_Header[5] = 'Total_Mean'
  FOR a=0, N_ELEMENTS(ROI_Names)-1 DO BEGIN ; Fill the output header array:
    IF a EQ 0 THEN b=6 ELSE b=b+2+N_ELEMENTS(Statistic_String) ; Update the array position index.
    File_Header[b] += RNS[a] + '_ValidCount'
    File_Header[b+1] += RNS[a] + '_NaNCount'
    FOR c=0, N_ELEMENTS(Statistic_String)-1 DO BEGIN
      File_Header[b+c+2] += RNS[a] + '_' + Statistic_String[c] ; Add the ROI and statistic names to the array.
    ENDFOR
  ENDFOR
  ...
  FOR i=0, N_ELEMENTS(In_Files)-1 DO BEGIN ; Loop through each input file.
    ...
    FOR j=0, N_ELEMENTS(ROI_Names)-1 DO BEGIN ; Loop through each input file.
      ...
      FOR k=0, N_ELEMENTS(Statistic_String)-1 DO BEGIN
        IF Statistic_String[k] EQ 'Mean' THEN Matrix_Statistics[j,s] = MEAN(Data_ROI, /NAN)
        IF Statistic_String[k] EQ 'Standard Deviation' THEN BEGIN
          IF Data_ValidCount LT 2 THEN Matrix_Statistics[j,s] = !VALUES.F_NAN ELSE Matrix_Statistics[j,s] =  STDDEV(Data_ROI, /NAN)
        ENDIF
        IF Statistic_String[k] EQ 'Variance' THEN BEGIN
          IF Data_ValidCount LT 2 THEN Matrix_Statistics[j,s] = !VALUES.F_NAN ELSE Matrix_Statistics[j,s] = VARIANCE(Data_ROI, /NAN)
        ENDIF
        IF Statistic_String[k] EQ 'Minimum' THEN Matrix_Statistics[j,s] = MIN(Data_ROI, /NAN)
        IF Statistic_String[k] EQ 'Maximum' THEN Matrix_Statistics[j,s] = MAX(Data_ROI, /NAN)
        IF Statistic_String[k] EQ 'Median' THEN Matrix_Statistics[j,s] = MEDIAN(Data_ROI, /EVEN)
        IF Statistic_String[k] EQ 'Sum' THEN Matrix_Statistics[j,s] = TOTAL(Data_ROI, 1, /NAN)
      ENDFOR
      ;-------------- ; Print information:
    ...
    Matrix_Out = MAKE_ARRAY(N_ELEMENTS(ROI_Names)*(N_ELEMENTS(Statistic_String)+2), /FLOAT) ; Create an array to hold output data.
    FOR a=0, N_ELEMENTS(ROI_Names)-1 DO BEGIN ; Fill the output array:
      IF a EQ 0 THEN b=0 ELSE b=b+2+N_ELEMENTS(Statistic_String) ; Update counter.
      ; FILL
      Matrix_Out[b] += Matrix_ValidCount[a]
      Matrix_Out[b+1] += Matrix_NaNCount[a]
      FOR c=0, N_ELEMENTS(Statistic_String)-1 DO BEGIN
        Matrix_Out[b+c+2] += Matrix_Statistics[a,c]
      ENDFOR
    ENDFOR
    ; Write to file::
    PRINTF, FORMAT='(10000(A,:,","))', UNIT_Out, i, '"'+ FNS_In +'"', '"'+ Date_String +'"', $
      STRTRIM(Combined_ValidCount, 2), STRTRIM(Combined_NaNCount, 2), STRTRIM(Combined_Mean, 2), STRTRIM(Matrix_Out, 2)
    FREE_LUN, UNIT_Out ; Close the output file.
    ;-------------------------------------------------------------------------------------------
