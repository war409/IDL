; ##############################################################################################
; NAME: FUNCTION_WIDGET_Set_Value_Conditional.pro
; LANGUAGE: IDL
...
;               
;               The widget was initially created to get the user defined no data status and if needed 
...
;-----------------------------------------------------------------------------------------------
PRO FUNCTION_WIDGET_Set_Value_Conditional_Event, EVENT ; This event handler responds to button events:
    ...
    'RADIO_BUTTON': BEGIN
      ...
      CASE buttonvalue OF
        0: BEGIN ; If 'Set a Grid Value to NaN' is selected:
          ; Create text widget:
          TEXT_LABEL = WIDGET_LABEL(INFO.BASE_B, VALUE='Enter Value:', /ALIGN_CENTER)
...
;***********************************************************************************************
; ##############################################################################################
FUNCTION FUNCTION_WIDGET_Set_Value_Conditional, TITLE=TITLE, ACCEPT_STRING=ACCEPT_STRING, DECLINE_STRING=DECLINE_STRING, $
  DEFAULT=DEFAULT, FLOATING=FLOATING, DOUBLE=DOUBLE, INTEGER=INTEGER, LONG=LONG
  ...
  IF N_ELEMENTS(ACCEPT_STRING) EQ 0 THEN ACCEPT_STRING = 'Set a Value'
  IF N_ELEMENTS(DECLINE_STRING) EQ 0 THEN DECLINE_STRING = 'Do Not Set a Value'
  ...
  RADIO = CW_BGROUP(BASE_A, [ACCEPT_STRING, DECLINE_STRING], $
  ...
  XMANAGER, 'FUNCTION_WIDGET_Set_Value_Conditional', PARENT ; Start XMANAGER
  ;---------------------------------------------------------------------------------------------  
  ;*********************************************************************************************  
