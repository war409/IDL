...
; DLM: 23/08/2011
;
; DESCRIPTION:  This program calculates MOD09A1 OWL version 0.2.2.
; 
...
;-----------------------------------------------------------------------------------------------
FUNCTION MOD09A1_Filenames, Day, Month, Year
...
PRO Batch_OWL0p2p2
  ...
  PRINT,''
  PRINT,'Begin Processing: Batch_OWL0p2p2'
  PRINT,''
  ;---------------------------------------------------------------------------------------------
  ; Input/Output:
  ;---------------------------------------------------------------------------------------------
  ...
  FOR i=0, DateCount-1 DO BEGIN ; FOR i
    ...
    ;-------------- ; Skip processing if at least one input file is corrupted.
    IF Corrupted EQ 0 THEN BEGIN
      ...
      ;-------------- ; Build output file
      OutputFilename = Out_Directory + Prefix + DOY_string + '.OWLv2' + '.img' ; Set the output filename.
      ...
      MixedIndex = BITWISE_OPERATOR(State, 3, 2, 1) ; Mixed cloud cells ["MIXED"= 0000000000000010] ; STATE = ((DATA AND BIN) EQ EQV)
      IF (N_ELEMENTS(MixedIndex) GT 1) THEN Matrix_Mask[MixedIndex] = 0
      ...
      FillIndexRed = WHERE(Red EQ -32768, CountFill) ; Fill cells
      IF (N_ELEMENTS(FillIndexRed) GT 1) THEN Matrix_Mask[FillIndexRed] = 0
      FillIndexNIR = WHERE(NIR EQ -32768, CountFill) ; Fill cells
      IF (N_ELEMENTS(FillIndexNIR) GT 1) THEN Matrix_Mask[FillIndexNIR] = 0
      FillIndexSWIR2 = WHERE(SWIR2 EQ -32768, CountFill) ; Fill cells
      IF (N_ELEMENTS(FillIndexSWIR2) GT 1) THEN Matrix_Mask[FillIndexSWIR2] = 0
      FillIndexSWIR3 = WHERE(SWIR3 EQ -32768, CountFill) ; Fill cells
      ...
      OWL2_BYTE = BYTE((OWL2 LT 255) * (OWL2+0.005) * 100.00 + (OWL2 EQ 255) * 255.00) ; Convert to byte.
