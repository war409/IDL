; ##############################################################################################
; NAME: Time_Series_Statistics_By_Month.pro
; LANGUAGE: IDL
...
PRO Time_Series_Statistics_By_Month
  ;---------------------------------------------------------------------------------------------
  ...
  ; Input/Output: 
  ;---------------------------------------------------------------------------------------------
  Prefix = 'MOD09Q1.MOD09A1.CMRSET.bias.correct.' ; Set a string prefix for the output file names.
  ;---------------------------------------------------------------------------------------------
  ...
  Path='C:\Users\war409\CMRSET\cmrset.bias.correct.monthly\'
...
;      Date_string = STRTRIM(iYear, 2) + M_string ; Set the output filename date string.
      ...
      FOR s=0, Count_S-1 DO BEGIN
        ...
        FOR f=0, N_ELEMENTS(Files_In)-1 DO BEGIN ; Loop through each input file.
          ...
          Matrix_Data[f,*] = Data_Segment * 0.001 ; Fill data array.
        ENDFOR
        ...
        IF In_Statistic[6] EQ 1 THEN BEGIN
          ...
          ;Data_Sum = TOTAL(Matrix_Data * dayfactor_Segment, 1, /NAN) ; MODIS AET 8-day Sum. Convert mean mm/d to mm/month.
          IF (Out_DataType EQ 2) THEN BEGIN ; Signed Integer:
