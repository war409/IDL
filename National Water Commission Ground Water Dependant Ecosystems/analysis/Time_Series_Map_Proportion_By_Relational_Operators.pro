; ##############################################################################################
; NAME: Time_Series_Map_Proportion_By_Relational_Operators.pro
; LANGUAGE: IDL
...
;               FUNCTION_WIDGET_Map_By_Frequency
;               
...
PRO Time_Series_Map_Proportion_By_Relational_Operators
  ;---------------------------------------------------------------------------------------------
  StartTime = SYSTIME(1) ; Get the current system time.
  PRINT,''
  PRINT,'Begin Processing: Time_Series_Map_Proportion_By_Relational_Operators'
  PRINT,''
  ;---------------------------------------------------------------------------------------------
  ; Input/Output:
  ;---------------------------------------------------------------------------------------------
  ...
  FOR s=0, Count_S-1 DO BEGIN
    ...
    IF In_Operation EQ 0 THEN BEGIN
      ...
      IF (Out_DataType EQ 1) THEN Data_Out = BYTE(Data_Out) ; Convert to Byte.
      IF (Out_DataType EQ 2) THEN Data_Out = FIX(Data_Out + 0.5) ; Convert to Integer.
      IF (Out_DataType EQ 3) THEN Data_Out = LONG(Data_Out + 0.5) ; Convert to LONG
      IF (Out_DataType EQ 5) THEN Data_Out = DOUBLE(Data_Out) ; Convert to DOUBLE
      IF (Out_DataType EQ 12) THEN Data_Out = UINT(Data_Out + 0.5) ; Convert to unsigned Integer.
    ENDIF
    ;-------------------------------------------------------------------------------------------
    ; Map the count of:
    IF In_Operation EQ 1 THEN BEGIN
      ...
      ;-------------- ; Datatype conversions:
      IF (Out_DataType EQ 1) THEN Data_Out = BYTE(Matrix_Data) ; Convert to Byte.
      IF (Out_DataType EQ 2) THEN Data_Out = FIX(Matrix_Data + 0.5) ; Convert to Integer.
      IF (Out_DataType EQ 3) THEN Data_Out = LONG(Matrix_Data + 0.5) ; Convert to LONG
      IF (Out_DataType EQ 5) THEN Data_Out = DOUBLE(Matrix_Data) ; Convert to DOUBLE
      IF (Out_DataType EQ 12) THEN Data_Out = UINT(Matrix_Data + 0.5) ; Convert to unsigned Integer.
    ENDIF
    ;-------------------------------------------------------------------------------------------
    ; Map by frequency:
    IF In_Operation EQ 2 THEN BEGIN
