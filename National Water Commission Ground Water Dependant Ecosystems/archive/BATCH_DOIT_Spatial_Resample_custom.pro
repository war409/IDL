; ##############################################################################################
; NAME: BATCH_DOIT_Spatial_Resample_custom.pro
; LANGUAGE: ENVI + IDL
...
PRO BATCH_DOIT_Spatial_Resample_custom
  ;---------------------------------------------------------------------------------------------
  ; GET START TIME
  T_TIME = SYSTIME(1)
  ;--------------
  PRINT,''
  PRINT,'BEGIN PROCESSING: BATCH_DOIT_Spatial_Resample_custom'
  PRINT,''
  ;---------------------------------------------------------------------------------------------  
  ;*********************************************************************************************
  ; INPUT/OUTPUT:
  ...
  FOR i=0, N_ELEMENTS(IN_FILES)-1 DO BEGIN ; START 'FOR i'
    ...
    ENVI_OPEN_FILE, FILE_IN, R_FID=FID_IN1, /NO_REALIZE
    ;--------------
    ; QUERY THE i-TH INPUT FILE
    ENVI_FILE_QUERY, FID_IN1, DIMS=DIMS_IN, DATA_TYPE=DATATYPE_IN, NS=NS_IN, NL=NL_IN
    ;--------------
    ; GET MAP INFORMATION
    MAPINFO_IN = ENVI_GET_MAP_INFO(FID=FID_IN1)
    ...
    ;--------------------------------------
    ; No Data
    DATA_TMP = FLOAT(TEMPORARY(DATA_IN))
    k = WHERE(DATA_TMP EQ FLOAT(NAN_VALUE), COUNT_k)
    IF (COUNT_k GT 0) THEN DATA_TMP[k] = !VALUES.F_NAN ; SET NAN
    ;--------------
    ; WRITE DATA_TMP
    ENVI_WRITE_ENVI_FILE, DATA_TMP, DATA_TYPE=4, /IN_MEMORY, INTERLEAVE=0, $
      MAP_INFO=MAPINFO_IN, NB=1, NL=NL_IN, NS=NS_IN, /NO_COPY, /NO_OPEN, $
      OUT_DT=4, PIXEL_SIZE=[XSIZE_IN,YSIZE_IN], R_FID=FID_IN
    ;--------------
    ; QUERY THE i-TH INPUT FILE
