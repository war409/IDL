; ##############################################################################################
; NAME: BATCH_DOIT_Temporal_Resample_SILO_to_8Day.pro
; LANGUAGE: IDL
...
PRO BATCH_DOIT_Temporal_Resample_SILO_to_8Day
  ;---------------------------------------------------------------------------------------------
  ; GET START TIME
  T_TIME = SYSTIME(1)
  ;--------------
  PRINT,''
  PRINT,'BEGIN PROCESSING: BATCH_DOIT_Temporal_Resample_SILO_to_8Day'
  PRINT,''
  ;---------------------------------------------------------------------------------------------  
  ;*********************************************************************************************
  ; INPUT/OUTPUT:
  ...
  PATH = '\\File-wron\TimeSeries\Climate\usilo\rain\'
  ...
  IF TYPE_NAN EQ 0 THEN NAN_VALUE = ENTER_VALUE_WIDGET('SET THE NODATA VALUE', -999.00)
  ;---------------------------------------------------------------------------------------------
  ...
  ENDIF ELSE BEGIN
    ...
    DMY = GET_FNAME_DMY_DDMMYYYY(FNS, 0, 4, 4, 2, 6, 2) 
  ENDELSE 
  ;---------------------------------------------------------------------------------------------   
  IF TYPE_INPUT EQ 1 THEN BEGIN
    ;-----------------------------------
    ...
    ENDIF ELSE BEGIN
      ; GET UNIQUE FILENAME DATES (FILENAME, YEAR_STARTPOS, YEAR_LENGTH, MONTH_STARTPOS, MONTH_LENGTH, DAY_STARTPOS, DAY_LENGTH) 
      DMY2 = GET_FNAME_DMY_DDMMYYYY(FNS2,  0, 4, 4, 2, 6, 2) 
    ENDELSE 
    ;-----------------------------------
  ENDIF
  ;---------------------------------------------------------------------------------------------
  ...
  WHILE (DEND = DSTART + DLENGTH) LE DSTOP DO BEGIN
    ...
    ENDIF ELSE BEGIN
      OUT_FNAME = OUT_DIRECTORY + PREFIX + '_' + OUT_FNAME_DATE + '.flt'
    ENDELSE
    ;--------------------------------------
    ...
    FOR s=0, COUNT_S-1 DO BEGIN ; s
      ...
      MATRIX_X = MAKE_ARRAY(N_ELEMENTS(FILES), SEGMENT_SIZE, /FLOAT)
      ;--------------------------------------
      ; START FILE LOOP:    
      ;-------------------------------------- 
      ...
      ENDELSE
      IF TYPE_NAN EQ 0 THEN BEGIN ; SET NAN
        k = WHERE(DATA_OUT EQ !VALUES.F_NAN, COUNT_k)
        IF (COUNT_k GT 0) THEN DATA_OUT[k] = NAN_VALUE
      ENDIF 
      ;-----------------------------------------------------------------------------------------
      ;*****************************************************************************************
      ; APPEND DATA:
  ...
  PRINT,''
  PRINT,'FINISHED PROCESSING: BATCH_DOIT_Temporal_Resample_SILO_to_8Day'
  PRINT,''
  ;---------------------------------------------------------------------------------------------
END