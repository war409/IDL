; ##############################################################################################
; NAME: Batch_DOIT_Temporal_Resample_8Day_to_Monthly.pro
; LANGUAGE: IDL
...
PRO Batch_DOIT_Temporal_Resample_8Day_to_Monthly
  ;---------------------------------------------------------------------------------------------
  T_TIME = SYSTIME(1) ; Get the program start time
  ;--------------
  PRINT,''
  PRINT,'BEGIN PROCESSING: Batch_DOIT_Temporal_Resample_8Day_to_Monthly'
  PRINT,''
  ;---------------------------------------------------------------------------------------------
  ;*********************************************************************************************
  ; Input/Output:
  ;*********************************************************************************************
  ;---------------------------------------------------------------------------------------------
  SILO_Mask = '\\File-wron\Working\work\NWC_Groundwater_Dependent_Ecosystems\Data\Raster\SILO\MODIS.Land.Mask.aust.005.250m.img'
  Mask = READ_BINARY(SILO_Mask, DATA_TYPE=1)
  ;---------------------------------------------------------------------------------------------
    ...
    ;-------------------------------------------------------------------------------------------  
      ...
      FOR s=0, COUNT_S-1 DO BEGIN ; FOR s 
        ...
        IF N_ELEMENTS(FILES_IN) GE 2 THEN BEGIN 
           ...
           IF METHOD_RESAMPLE EQ 4 THEN DATA_OUT = TOTAL(MATRIX_X, 1, /NAN) * ( (N_ELEMENTS(FILES_IN)*8)/eDay) ; dont forget no datas!
        ENDIF ELSE BEGIN
          DATA_OUT = READ_BINARY(FILES_IN[0], DATA_TYPE=IN_DT) ; Set out data.
        ENDELSE
        ;--------------
        k = WHERE(Mask(SEGMENT_START_TMP:SEGMENT_END_TMP) EQ 0, COUNT_k)
  ...
  PRINT,''
  PRINT,'FINISHED PROCESSING: Batch_DOIT_Temporal_Resample_8Day_to_Monthly'
  PRINT,''
  ;---------------------------------------------------------------------------------------------
END      
