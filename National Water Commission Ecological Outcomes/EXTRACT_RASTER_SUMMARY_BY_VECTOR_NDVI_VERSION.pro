; ######################################################################
; NAME: EXTRACT_RASTER_SUMMARY_BY_VECTOR_NDVI_VERSION.pro
; LANGUAGE: IDL
; AUTHOR: Garth Warren 
; DATE: 31/09/2009
; DLM: 16/10/2009
...
;              The NDVI (mNDWI) version features a modified output. Summary
...
; ######################################################################
;
PRO EXTRACT_RASTER_SUMMARY_BY_VECTOR_NDVI_VERSION
  PRINT,''
  PRINT,'BEGIN PROCESSING: EXTRACT_RASTER_SUMMARY_BY_VECTOR_NDVI_VERSION'
  PRINT,'' 
  ;
  ...
  FOR i=0, COUNT-1 DO BEGIN
    ...
    FOR j=0, N_ELEMENTS(POS)-1 DO BEGIN
      ...
      PRINTF, FORMAT='(10000(A,:,","))', OUTLUN, j, '"' + NEWBNAME + '"', '"' + ROI_NAME + '"', $
        OUTMEANX, OUTSTDDEVX, OUTMEDIANX, OUTMINX, OUTMAXX
      ;
  ...
  ;
  PRINT,''
  PRINT,'FINISHED PROCESSING: EXTRACT_RASTER_SUMMARY_BY_VECTOR_NDVI_VERSION'
  PRINT,''
END